...
class TicketSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for tr in trs:
            ...
            description = tr.xpath(".//div[1]/div[@class='exercise']/text()").get().strip()
            grade = tr.xpath(".//div[1]/div[@class='search_ticket_assess']/span[1]/em/text()").get().strip()
            grade = str(grade)
            price = tr.xpath("normalize-space(.//table/tbody/tr[2]/td[4]/span[1])").get().strip()
            price = str(price)
            supplier = '00003'
